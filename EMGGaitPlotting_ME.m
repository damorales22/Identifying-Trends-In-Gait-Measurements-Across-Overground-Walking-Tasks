...
for i = 1:length(labels) % iterate through muscles
    ...
    for j = 1:numtrials % plot trials
        ...
        [trials.(trialname).segments, trials.(trialname).average] = EMGProcess(EMGdata, strikes, fs, f_high, f_low, f_lowpass, n, m, i, side, maxsteptime); %store segments and average respectivelly
...
function[segments, avg] = EMGProcess(EMGData, strikes, fs, f_high, f_low, f_lowpass, n, m, channel, side, maxsteptime)
    ...
    trialtime = numframes/framerate; %total duration of the trial
    ...
    %% Segment EMG data
    num_segments = length(strikes.(side).secs) - 1; % One less than the total timesteps
    maxlength = fs * maxsteptime;
    segments = arrayfun(@(i) segmentEMGData(EMG_filtered, strikes.(side).secs(i), strikes.(side).secs(i+1), fs, maxlength), 1:num_segments, 'UniformOutput', false); %arrayfun: applies a specified function to each element of an array
                                                                 %start                    end                         1:num_segments => range of indices that arrayfun will iterate over        .                                                               
    % Filter out empty segments; normally one at the end
    segments = segments(~cellfun(@isempty, segments));

    % create steps (based on longest segment)
    num_segments = length(segments);
    max_length = max(cellfun(@length, segments)); % find longest segment to interpolate later
    steps = (0:max_length-1) / max_length * 100; %normalize (*100 so we have 0-100) to create the points in the x axis we will plot after interpolation
...
function segment = segmentEMGData(EMG_filtered, start_sec, end_sec, fs, maxlength)
    ...
    start_idx = round(start_sec * fs); % Start index of segment
    end_idx = round(end_sec * fs); % End index of segment
    steptime = end_idx - start_idx; %number of frames of the step
